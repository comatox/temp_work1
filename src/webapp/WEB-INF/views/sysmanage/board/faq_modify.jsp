...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    ...
    let form = document.faqModifyForm;
    boardNo = "${param.boardNo}";

    if (boardNo) {
      $.esutils.renderData(
          "faqModifyForm",
          `/api/sysmanage/faq/detail/\${boardNo}`,
          (data) => {
  ...
  const fileUpdate = function () {
    ...
    $.ajax({
      url: global.contextPath + '/api/sysmanage/faq/deleteAjax',
      type: 'post',
   ...
   * 저장
   ***************************************************************************/
  const faqSave = function (fileInfo) {
    if (!validationCheck()) return;
    if (!confirm("수정 하시겠습니까?")) return;

    const formObject = document.getElementById('faqModifyForm');

    $.esutils.multipartSubmit(
        '/api/sysmanage/faq/modify',
        formObject,
        function (formData) {
  ...
  const validationCheck = function () {
    ...
    document.faqModifyForm.content.value = CKEDITOR.instances.ir1.getData();
        ...
        <div id="content_area">
            <div id="contentsArea">
                ...
                <div id="realContents">
                    ...
                    <form id="faqModifyForm" name="faqModifyForm" method="post" enctype="multipart/form-data">
                        ...
                        <div class="buttonGroup">
                            <div class="leftGroup">
                                ...
                                <span class="button bt_s1"><button type="button" style="width:50px;" onclick="faqSave();" tabindex="6">수정</button></span>
                            </div>
                        </div>
                        <!-- 버튼 끝 -->
                    </form>
