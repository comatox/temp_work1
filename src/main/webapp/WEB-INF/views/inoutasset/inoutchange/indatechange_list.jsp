...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    $("[name=fromWritedate]").val(moment().subtract(1, "years").format("YYYY-MM-DD"));
    ...
    gridUtil = new GridUtil({
      url: "/api/inoutasset/inoutchange/indatechange"
      , isPaging: true
      , gridOptions: {
        colData: [
          {
          ...
          },
          {
            headerName: "작성자"
            , field: "empNm"
            , formatter: (_, __, row) => `\${row.deptNm} \${row.empNm}`
          },
          {
            headerName: "지연여부"
            , field: "dateDiff"
      ...
      search: {
        formId: "formGrid"
        , buttonId: "searchBtn"
        , beforeSend: data => ({...data, searchPartnerNm: $("[name=searchOutcompanyknd]").val() === "3" ? $("[name=searchForeignComp]").val() : $("[name=searchDeptNm]").val()})
      }
    });
  ...
  function fn_view({inoutApplNo}) {
    $.esutils.href("/inoutasset/inoutchange/indatechange/detail", {inoutApplNo});
  }
</script>
<div id="search_area">
    <!-- 검색 -->
...
<div style="color: red;">※ 반입예정일자 변경은 3개월 이내로, 작성일로부터 최대 1년까지 연장할 수 있습니다.</div>
<!-- 그리드 영역 시작 -->
<div id="grid"></div>
<!-- 그리드 영역 종료 -->