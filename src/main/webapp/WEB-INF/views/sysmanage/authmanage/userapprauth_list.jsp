...
<script type="text/javascript">
  ...
  function viewPopup({originRowData}) {
    ...
    const {jcCd, jwId, empId: searchEmpId} = originRowData;
    const jcCdArray = ["JZ", "39", "60", "17", "72", "76", "AA", "80", "B0", "B2", "B1", "KA", "O1", "GA"];
    const jwIdArray = ["UB"];

    if (jcCdArray.includes(jcCd) || jwIdArray.includes(jwId)) {
      const param = {searchEmpId: originRowData.empId, managementKnd: $("#managementKnd").val(), authKnd: "2"};
      $.esutils.openPopup({
      ...
      alert("직책이 팀장이상이 아닌경우 요청부서 결재선 설정을 할 수 없습니다.");
    }
  }
...
<div id="realContents">
    <form id="userAuthManageForm" name="userAuthManageForm">
        ...
        <input type="hidden" id="searchAuthKnd" name="searchAuthKnd" value="2"/>
