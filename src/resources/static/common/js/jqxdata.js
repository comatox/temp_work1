...
(function(a) {
	...
	a.jqx.dataAdapter.prototype = {
		...
		dataBind : function(l, q) {
			...
			this.records = new Array();
			var f = this._options || {};
			this.virtualmode = f.virtualmode != undefined ? f.virtualmode
					: false;
			this.totalrecords = f.totalrecords != undefined ? f.totalrecords
					: 0;
			...
			this.cachedrecords = f.cachedrecords != undefined ? f.cachedrecords
					: new Array();
			...
			var v = this;
			var k = n.datatype;
			if (n.datatype === "csv" || n.datatype === "tab"
					|| n.datatype == "text") {
				k = "text"
			}
			...
			default:
				...
				this.totalrecords = this.virtualmode ? (n.totalrecords || g)
						: g;
				if (this.unboundmode) {
					this.totalrecords = this.unboundmode ? (n.totalrecords || g)
							: g;
										...
										if (t > 0) {
											...
											for ( var z in this) {
												...
												if (A) {
													w[z] = this[z]();
													if (this[z].subscribe) {
														v.koSubscriptions[v.koSubscriptions.length] = this[z]
																.subscribe(function(
																		B) {
																	v
																			.dataBind(
				...
				if (f.uniqueDataFields) {
					var m = this.getUniqueRecords(this.records,
							f.uniqueDataFields);
					this.records = m;
			...
			case "text":
				if (n.localdata != null) {
					...
					if (f.uniqueDataFields) {
						var m = v.getUniqueRecords(v.records,
								f.uniqueDataFields);
						v.records = m;
						v.cachedrecords = m
					}
					...
					if (a.isFunction(f.loadServerData)) {
						...
						a.ajax({
							...
							success : function(z, w, B) {
								...
								if (f.uniqueDataFields) {
									var y = v.getUniqueRecords(v.records,
											f.uniqueDataFields);
									v.records = y;
									v.cachedrecords = y
								}
		...
		getAggregatedData : function(m, g, e) {
			var d = e;
			if (!d) {
				d = this.records
			}
						...
						if (typeof n === "number" && isFinite(n)) {
							a.each(b.aggregates, function() {
								...
								if (this == "product") {
									...
									if (this == "min") {
										o = Math.min(o, parseFloat(n))
									} else {
										if (this == "max") {
											o = Math.max(o, parseFloat(n))
										} else {
											if (this == "count") {
												o++
											} else {
			...
			for ( var f = 0; f < m.length; f++) {
				...
				if (h[b.name]["avg"]) {
					var n = h[b.name]["avg"];
					h[b.name]["avg"] = n / d.length
				} else {
					if (h[b.name]["count"]) {
						h[b.name]["count"] = c
					}
				}
				if (h[b.name]["stdev"] || h[b.name]["stdevp"]
						|| h[b.name]["var"] || h[b.name]["varp"]) {
					a.each(b.aggregates, function(u) {
						if (this == "stdev" || this == "var" || this == "varp"
								|| this == "stdevp") {
							var v = h[b.name][this];
							var t = c;
							var o = (v / c);
				...
				if (b.formatStrings) {
					a.each(b.aggregates, function(p) {
						var o = b.formatStrings[p];
						if (o) {
							var q = h[b.name][this];
							h[b.name][this] = a.jqx.dataFormat.formatnumber(q,
									o, g)
						}
					})
				}
		...
		getGroupedRecords : function(D, F, r, y) {
			var A = 0;
			var w = this;
			var d = new Array();
		...
		loadjson : function(v, w, m) {
			...
			this.originaldata = new Array();
			var q = this.records;
			var n = !this.pageable ? m.recordstartindex : this.pagesize
					* this.pagenum;
			this.recordids = new Array();
			if (m.loadallrecords) {
				n = 0;
			...
			var k = 0;
			if (this.virtualmode) {
				n = !this.pageable ? m.recordstartindex : this.pagesize
						* this.pagenum;
				k = n;
				n = 0;
				e = this.totalrecords
			...
			.extend(
					a.jqx.dataFormat,
					{
						...
						getDayIndex : function(f, e, c) {
							...
							if (!d) {
								f._upperDays = d = [
										this.toUpperArray(g.names),
										this.toUpperArray(g.namesAbbr),
										this.toUpperArray(g.namesShort) ]
							}
							e = toUpper(e);
							if (c) {
						...
						getMonthIndex : function(k, h, d) {
							...
							if (!f) {
								k._upperMonths = f = [
										this.toUpperArray(b.names),
										this.toUpperArray(b.namesAbbr) ];
								k._upperMonthsGen = g = [
										this.toUpperArray(c.names),
										this.toUpperArray(c.namesAbbr) ]
							}
							h = this.toUpper(h);
