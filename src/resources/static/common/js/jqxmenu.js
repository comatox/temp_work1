			...
			.extend(
					a.jqx._jqxMenu.prototype,
					{
						defineInstance : function() {
							...
							this.touchMode = "auto";
							this.source = null;
							this.events = [ "shown", "closed", "itemclick",
									"initialized" ]
						},
						createInstance : function(d) {
							var c = this;
							this.host.css("display", "block");
							this.propertyChangeMap.disabled = function(h, k, j,
									l) {
								if (c.disabled) {
									c.host
											.addClass(c
							...
							var f = this;
							if (f.width != null
									&& f.width.toString().indexOf("%") != -1) {
								b = true
							}
							if (f.height != null
									&& f.height.toString().indexOf("%") != -1) {
								b = true
							}
							a(window).bind(
									"resize.menu" + this.element.id,
									function() {
										if (b) {
											if (f.refreshTimer) {
												clearTimeout(f.refreshTimer)
											}
											f.refreshTimer = setTimeout(
													function() {
														f.refresh()
													}, 1)
										}
															...
															if (o.level > 0) {
																...
																if (p.length > 0) {
																	...
																	if (o.openHorizontalDirection != "left") {
																		p
																				.addClass(l
																						.toThemeProperty("jqx-menu-item-arrow-right"))
																	} else {
																		p
																				.addClass(l
																						.toThemeProperty("jqx-menu-item-arrow-left"))
																	}
																}
															} else {
																a(o.element)
																...
																if (p.length > 0) {
																	...
																	if (o.openHorizontalDirection != "left") {
																		p
																				.addClass(l
																						.toThemeProperty("jqx-menu-item-arrow-top-right"))
																	} else {
																		p
																				.addClass(l
																						.toThemeProperty("jqx-menu-item-arrow-top-left"))
																	}
																}
															}
															k
								...
								if (g != undefined && g) {
									...
									a
											.each(
													d,
													function() {
														if (l.menuElements[this.id]
																&& l.menuElements[this.id].isOpen) {
															var p = a(l.menuElements[this.id].subMenuElement);
															l
																	._closeItem(
																			l,
																			l.menuElements[this.id],
																			true,
																			true)
														}
													})
						...
						_openItem : function(n, q, p) {
							...
							var h = a(q.element);
							var g = q.level == 0 ? this._getOffset(q.element)
									: h.position();
							...
							var r = b ? g.left
									: this.menuElements[q.parentId] != null
											&& this.menuElements[q.parentId].subMenuElement != null ? parseInt(a(
											a(
													this.menuElements[q.parentId].subMenuElement)
													.closest(
															"div.jqx-menu-popup"))
											.outerWidth())
											- e[0]
												...
												if (q.level > 0) {
													...
													if (s.length > 0) {
														...
														if (q.openHorizontalDirection != "left") {
															s
																	.addClass(n
																			.toThemeProperty("jqx-menu-item-arrow-right-selected"))
														} else {
															s
																	.addClass(n
																			.toThemeProperty("jqx-menu-item-arrow-left-selected"))
														}
													}
												} else {
													h
						...
						_applyOrientation : function(h, c) {
							...
							this.host
									.removeClass(f.toThemeProperty("jqx-menu"));
							this.host.removeClass(f
							...
							if (this.autoOpenPopup) {
								if (this.mode == "popup") {
									a(document).bind("contextmenu",
											function(j) {
												return false
											});
									a(document).bind(
											"mousedown.menu" + this.element.id,
											f, this._openContextMenu)
								} else {
										...
										.each(
												...
												function() {
													...
													if (l.level == 0) {
														...
														if (!l.ignoretheme
																&& l.hasItems
																&& f.showTopLevelArrows) {
															var k = a('<span style="border: none; background-color: transparent;" id="arrow'
																	+ $element[0].id
																	+ '"></span>');
															var j = a.browser.msie
																	&& a.browser.version < 8;
															if (l.arrow == null) {
																if (!j) {
																	k
										...
										.each(
												...
												function() {
													...
													if (k.hasItems
														...
														var j = a('<span style="border: none; background-color: transparent;" id="arrow'
																+ $element[0].id
																+ '"></span>');
														j.prependTo($element);
														j.css("float", "right");
						...
						_getOffset : function(c) {
							...
							var d = a.jqx.mobile.isSafariMobileBrowser();
							var h = a(c).offset();
							var f = h.top;
							var e = h.left;
							if (d != null && d) {
						...
						_render : function(f, b) {
							...
							var c = [ 5, 5 ];
							var e = this;
							a.data(e.element, "animationHideDelay",
									e.animationHideDelay);
							...
							if (this.autoCloseOnClick) {
								...
								this.addHandler(a(document), "mousedown.menu"
										+ this.element.id, e._closeAfterClick,
										e);
								this.removeHandler(a(document), "mouseup.menu"
										+ this.element.id, e._closeAfterClick);
								this.addHandler(a(document), "mouseup.menu"
										+ this.element.id, e._closeAfterClick,
										e);
								if (d) {
									this
											.addHandler(a(document),
													"touchstart.menu"
															+ this.element.id,
													e._closeAfterClick, e)
								}
							}
							this._applyOrientation(f, b);
							if (e.enableRoundedCorners) {
								this.host.addClass(e
									...
									.each(
											...
											function() {
												...
												e
														.addHandler(
																...
																function(u) {
																	...
																	if (u.target.tagName != "A"
																			&& u.target.tagName != "a") {
																		var s = p.anchor != null ? a(p.anchor)
																				: null;
																		if (s != null
																				&& s.length > 0) {
																			var r = s
												...
												if (p.subMenuElement != null) {
													...
													m
															.wrap('<div class="jqx-menu-popup" style="border: none; background-color: transparent; z-index:'
																	+ g
						...
						_createMenu : function(l, n) {
							...
							for ( var m = 0; m < e.length; m++) {
								if (e[m].className.indexOf("jqx-menu") == -1
								...
								if (n) {
									...
									b.anchor = g.parents("li").length == b.level + 1 ? g
											: null
								}
								b.ignoretheme = o;
						...
						destroy : function() {
							...
							a.each(this.items, function() {
								var e = this;
								var d = a(e.element);
								b.removeHandler(d, "click");
								b.removeHandler(d, "selectstart");
								b.removeHandler(d, "mouseenter");
								b.removeHandler(d, "mouseleave");
								b.removeHandler(d, "mousedown");
								b.removeHandler(d, "mouseleave")
							});
							this.host.remove()
						},
						...
						_refreshStyle : function() {
							...
							this.host.addClass(b.toThemeProperty("jqx-widget"));
							this.host.addClass(b
									.toThemeProperty("jqx-widget-header"));
							a
									.each(
											this.items,
